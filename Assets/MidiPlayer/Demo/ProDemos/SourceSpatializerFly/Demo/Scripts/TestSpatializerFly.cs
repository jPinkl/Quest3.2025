    ...
    public class TestSpatializerFly : MonoBehaviour
    {
        ...
        public void ArrangeByInstrument()
        {
            ...
            foreach (MidiFilePlayer mfp in MidiFilePlayer.SpatialSynths)
            {
                ...
                TestSpatializerFly tsf = mfp.gameObject.GetComponent<TestSpatializerFly>();
                int preset = mfp.MPTK_ChannelPresetGetIndex(mfp.MPTK_SpatialSynthIndex);
                // Appply a 3D position depending the current preset
                tsf.PosSynth = PositionByInstrument(tsf.PosSynth, preset, mfp.MPTK_SpatialSynthIndex);
            }
        }
                ...
                else
                {
                    ...
                    if (MidiMaster.MPTK_ModeSpatializer == MidiSynth.ModeSpatializer.Channel)
                    {
                        if (midiSpatializer.MPTK_SpatialSynthIndex == 9)
                            ...
                            textPlayer.text = midiSpatializer.MPTK_ChannelPresetGetName(midiSpatializer.MPTK_SpatialSynthIndex);
                    }
                    else
                    {
                    ...
                    if (DisplayDebugInfo)
                        textPlayer.text += $"\nId{midiSpatializer.MPTK_SpatialSynthIndex + 1} P:{midiSpatializer.MPTK_ChannelPresetGetIndex(midiSpatializer.MPTK_SpatialSynthIndex)} N:{CountNote}";
