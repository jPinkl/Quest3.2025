...
namespace MidiPlayerTK
{
    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        /// <param name="resetActiveVoices">Not more used</param>
        public void MPTK_InitSynth(int channelCount = 16, bool resetActiveVoices = false)
        {
            ...
            for (int i = 0; i < Channels.Length; i++)
            {
                ...
                Channels[i].mptkChannel = MptkChannels[i];
        ...
        protected void ResetMPTKChannels()
        {
            ...
            {
                foreach (mptk_channel channel in MptkChannels)
                {
        ...
        /// @snippet TestMidiFilePlayerScripting.cs ExampleUsingChannelAPI
        public string MPTK_ChannelInfo(int channel)
        {
            ...
            {
                string sPreset = "";
                int presetForced = MPTK_ChannelForcedPresetGet(channel);
                if (presetForced == -1)
                {
                    // Preset not forced, get the preset defined on this channel by the Midi
                    sPreset = MPTK_ChannelPresetGetIndex(channel).ToString();
                }
                else
                {
                ...
                string sMuted = MPTK_ChannelEnableGet(channel) ? "Enabled" : "Muted";

                info += $"{channel}\t";
                info += $"{sMuted}\t";
                info += $"[Preset:{sPreset}, ";
                info += $"Bank:{MPTK_ChannelBankGetIndex(channel)}]\t";
                info += $"'{MPTK_ChannelPresetGetName(channel)}'\t";
                info += $"Count:{MPTK_ChannelNoteCount(channel)}\t";
                info += $"Volume:{MPTK_ChannelVolumeGet(channel):F2}\t";
            }
            return info;
        }
        ...
        /// Have a look to the demo: Assets\MidiPlayer\Demo\FreeMVP\MidiLoop.cs
        /// @snippet MidiLoop.cs ExampleFindPlayerAndAddListener
        /// @snippet MidiLoop.cs ExampleChannelEnabled
        /// </summary>
        /// <param name="channel">must be between 0 and 15 to apply on one channel, -1 to apply to all channels from 0 to 15 v2.9.0</param>
        /// <param name="enable">true to unmute (default), false to mute channel</param>
        public void MPTK_ChannelEnableSet(int channel, bool enable)
        {
            if (MptkChannels != null)
            {
                ...
                {
                    for (int i = 0; i < 16; i++)
                        MptkChannels[i].enabled = enable;
                }
                else
                {
                    if (channel >= 0 && channel < MptkChannels.Length)
        ...
        /// <param name="volume">percentage of volume from the global volume for the channel, must be between 0 and 1</param>
        public void MPTK_ChannelVolumeSet(int channel, float volume)
        {
            if (MptkChannels != null && channel < MptkChannels.Length)
                if (channel >= 0)
                    MptkChannels[channel].volume = volume;
                else if (channel == -1)
                    for (int chan = 0; chan < 16; chan++)
                        MptkChannels[chan].volume = volume;
        ...
        /// </summary>
        /// <param name="channel">must be between 0 and 15</param>
        /// <returns>channel preset index or -1 if channel error</returns>
        ...
        /// </summary>
        /// <param name="channel">MIDI channel must be between 0 and 15</param>
        /// <param name="controller">MIDI Controller must be between 0 and 127. See MPTKEvent#Controller</param>
        /// <returns>controller value or 0 if channel error</returns>
        ...
        /// <param name="preset">Preset to force in the range 0 to 127 preset. Set to -1 to restore the Preset and Bank known from the MIDI file.</param>
        /// <param name="bank">Optionnal. Bank to force in the range 0 to 128. Set to -1 to restore to the last bank known from the MIDI file.</param>
        /// <returns>true if the preset/bank exists in the current SoudFont, otherwise false</returns>
        /// 
        /// @code
        ...
        public bool MPTK_ChannelForcedPresetSet(int channel, int preset, int bank = -1)
        {
            ...
            if (CheckParamChannel(channel))
            {
                ...
                if (bank >= 0)
                {
                    MptkChannels[channel].forcedBank = bank; // set to -1 to disable forced bank
                }
                ...
                {
                    MptkChannels[channel].forcedPreset = -1;
                    MptkChannels[channel].forcedBank = -1;
                    preset = MptkChannels[channel].lastPreset;
                    Channels[channel].banknum = MptkChannels[channel].lastBank;
                }
        ...
        /// @snippet TestMidiStream.cs Example MPTK_ChannelPresetChange
        /// </summary>
        /// <param name="channel">0 to 15. There is 16 channels available in the MIDI norm.</param>
        /// <param name="preset">The total number of presets available depend on the soundfont selected</param>
        /// <param name="bank">Optionnal, if not set or -1 the current bank will be applied (or default bank if never set). Default bank is defined globally: menu MPTK / SoundFont Setup.</param>
        /// <returns>true if the bank and the preset were found else false</returns>
        public bool MPTK_ChannelPresetChange(int channel, int preset, int bank = -1)
        {
            if (CheckParamChannel(channel))
            {
                ...
                MptkChannels[channel].lastPreset = preset;
                MptkChannels[channel].lastBank = selectedBank;
                fluid_synth_program_change(channel, preset);
                return ret;
            }
            return false;
        ...
        public fluid_voice fluid_synth_alloc_voice(HiSample hiSample, int chan, int _idSession, int key, int vel)
        {
            ...
            for (int indexVoice = 0; indexVoice < FreeVoices.Count;)
            {
                ...
                if (freeVoice.sample.Name == hiSample.Name && _idSession == freeVoice.IdSession)
                {
                    voice = freeVoice;
        ...
        {
            if (banknum >= 0 && banknum < sfont.Banks.Length && sfont.Banks[banknum] != null)
            {
                        ...
                        foreach (HiZone inst_zone in inst.Zone)
                        {
                                ...
                                (inst_zone.VelHi >= vel))
                            {
                                ...
                                if (MPTK_LogWave)
                                    ...
                                    note.Channel + 1,
                                    Channels[note.Channel].banknum,
                                    Channels[note.Channel].prognum,
                                    preset.Name,
                                    key,
        ...
        public void fluid_synth_program_change(int pchan, int preset)
        {
            ...
            if (pchan != 9 || MPTK_EnablePresetDrum == true) // V2.89.0
            {
                if (MptkChannels[pchan].forcedPreset >= 0)
                    preset = MptkChannels[pchan].forcedPreset;

                channel = Channels[pchan];

                banknum = MptkChannels[pchan].forcedBank >= 0 ? MptkChannels[pchan].forcedBank : channel.banknum; //fluid_channel_get_banknum
                ...
                channel.preset = hiPreset; // fluid_channel_set_preset
            }
        }
            ...
            try
            {
...
#endif
                if (MPTK_LogEvents)
                    Debug.Log($"MIDI Synth: {midiEvent}");
                ...
                switch (midiEvent.Command)
                {
                    ...
                    case MPTKCommand.PatchChange:
                        ...
                        {
                            MptkChannels[midiEvent.Channel].lastPreset = midiEvent.Value;
        ...
        private void OnAudioFilterRead(float[] data, int channels)
        {
            ...
            if (histoCurrent >= histoDspSize.Length) histoCurrent = 0;
            //if (data.Length % FLUID_BUFSIZE != 0)
            //{
            //    Debug.LogWarning($"OnAudioData DspBufferSize:{data.Length} not a multiple of FLUID_BUFSIZE:{FLUID_BUFSIZE}");
            //    return;
            //}
#endif
        ...
        void PlayMidi()
        {
            ...
            // 2.9.0  removed EllapseMidi = watchMidi.ElapsedMilliseconds;
            // Read midi events until this time
            List<MPTKEvent> midievents = midiLoaded.fluid_player_callback((int)timeMidiFromStartPlay/* removed 2.9.0 EllapseMidi*/, IdSession);
