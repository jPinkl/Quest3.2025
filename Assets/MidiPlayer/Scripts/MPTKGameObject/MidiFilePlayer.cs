...
namespace MidiPlayerTK
{
    ...
    public partial class MidiFilePlayer : MidiSynth
    {
        ...
        /// @note
        /// @li No impact on #MPTK_Duration that remains the same, so there is a shift between the real time of the MIDI and the theoretical duration.
        /// </summary>
        //[HideInInspector]
        public bool MPTK_StartPlayAtFirstNote;

        /// <summary>
        /// Defined how the MIDI player is stopped or looped at the last note.
        /// </summary>
        public enum ModeStopPlay
        {
            /// <summary>
            /// Stop or loop immeadiately (a short delay could occurs related to the Unity FPS).
            /// </summary>
            StopNoWaiting,
            /// <summary>
            /// Stop or loop when all notes start the release phase (when note-of occurs or the duration is reaches).
            /// </summary>
            StopWhenAllVoicesReleased,
            /// <summary>
            /// Stop or loop when all notes are in the ended phase (after the release phase). This delay is dependant of the instrument.
            /// A drum has a short release phase, a piano a medium delay, a tubullar bells a very long delay. 
            /// </summary>
            StopWhenAllVoicesEnded,
        }
        public string[] ModeStopPlayLabel = { "Stop No Waiting", "Stop When All Voices Are Released", "Stop When All Voices Are Ended" };
        /// <summary>@brief 
        /// Defined how the MIDI player is stopped or looped at the last note.
        /// @version 2.9.1
        /// @note
        /// @li Apply only if the last event + duration is a note-on.
        /// @li No impact on #MPTK_Duration that remains the same (including the duration of the last note).
        /// </summary>
        //[HideInInspector]
        ...
        /// The MIDI file doesn't need to be reloaded, the looping is almost immediate (one Unity frame).\n
        /// It's also possible to defined start / end looping position with attributes from MPTK_MidiLoaded.MPTK_TickStart and MPTK_MidiLoaded.MPTK_TickEnd
        ...
        /// <summary>@brief 
        /// Set or get the current tick in milliseconds.
        /// @note
        /// @li Works only when the MIDI is playing.
        /// @li You can't set the tick before or just after MPTK_Play() because MPTK_Play() reset the tick at the start.
        /// @li Rather, set the tick when the event OnEventStartPlayMidi() is triggered. See example bellow.
        /// @li Look also the properties #MPTK_TickCurrent to change the tick by MIDI ticks.
        /// @li When the MIDI is playing look at the inspector of the MidiFilePlayer prefab to read (or change) the current tick in seconds and find the tick you want.
        /// @par
        /// @details
        ...
        /// @li works only when the MIDI is playing.\n
        /// @li you can't set the tick before or just after MPTK_Play() because MPTK_Play() reset the tick at the start.\n
        /// @li rather, set  the tick when the event OnEventStartPlayMidi() is triggereed. See example below.\n
        /// @li look also the properties #MPTK_Position to change the tick by milliseconds.\n
        /// @li when the MIDI is playing look at the inspector of the MidiFilePlayer prefab to read (or change) the current tick and find the tick you want.\n
        /// @par
        /// See example:\n
        /// @code
        /// public MidiFilePlayer midiFilePlayer;
        ...
        /// @note
        /// @li Depends on the current tempo (#MPTK_CurrentTempo) and the #MPTK_DeltaTicksPerQuarterNote and Speed.
        /// @li Read Only.
        ...
        /// @note
        /// @li With v2.9.0 this event is trigerred even if the note is still in play. That could be unpleasant if you want yo loop on the MIDI.
        /// @li Enable "Keep MIDI End Track" (MPTK_KeepEndTrack) or "Keep Note Off" (MPTK_KeepNoteOff) when loading the MIDI file to sync the end of the playing with the real end of the MIDI.
        /// @par
        /// @code
        /// 
        /// using MidiPlayerTK; // Add a reference to the MPTK namespace at the top of your script
        ...
        /// <summary>@brief 
        /// Stop playing immediately and clear all sound
        /// </summary>
        ...
        {
            // before v2.9.0 MidiLoad miditoload = new MidiLoad();
