...
namespace MidiPlayerTK
{
    ...
    public class MidiCommonEditor : ScriptableObject
    {
        ...
        {
            if (MPTKGui.myStyle == null) MPTKGui.myStyle = new CustomStyle();
            EditorGUILayout.BeginVertical(MPTKGui.myStyle.LabelTitle);
            EditorGUILayout.BeginHorizontal();
        ...
        public void MidiFileParameters(MidiFilePlayer instance)
        {
            ...
            instance.MPTK_PlayOnStart = EditorGUILayout.Toggle(new GUIContent("Play At Startup", tooltip), instance.MPTK_PlayOnStart);

            tooltip = "Start playing Midi from the first note found in the MIDI.\n";
            instance.MPTK_StartPlayAtFirstNote = EditorGUILayout.Toggle(new GUIContent("Start Play From First Note", tooltip), instance.MPTK_StartPlayAtFirstNote);
            ...
            if (EditorGUILayout.DropdownButton(new GUIContent(instance.ModeStopPlayLabel[(int)instance.MPTK_ModeStopVoice], tooltip), FocusType.Passive)) //,new GUIStyle( "MiniPopup")))
            {
                var dropDownMenu = new GenericMenu();
                foreach (ModeStopPlay mode in Enum.GetValues(typeof(ModeStopPlay)))
                    dropDownMenu.AddItem(
                        new GUIContent(instance.ModeStopPlayLabel[(int)mode], ""),
        ...
        public void SynthParameters(MidiSynth instance, SerializedObject sobject)
        {
            ...
            if (instance.showSynthParameter)
            {
                ...
                instance.MPTK_LogEvents = EditorGUILayout.Toggle(new GUIContent("Log MIDI Events Played", "Log information about each MIDI events played."), instance.MPTK_LogEvents);
